"""
Repository for content embeddings that works with the CSV schema.
"""
...
class ContentEmbeddingRepository:
    ...
    def get_similar_by_ingredients(
        ...
        """
        if content_type != 'meal' and content_type != 'recipe':
            return []
        ...
        for row in result:
            items.append({
                ...
                "content_type": content_type,
                "title": row[1],
                "similarity": float(row[3])  # Convert to float to ensure JSON serialization
            })
