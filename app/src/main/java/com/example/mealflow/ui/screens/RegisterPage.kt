    ...
    fun handleRegistration() {
        if (viewModel.validateInputs(username, email, password)) {
            coroutineScope.launch {
                registerUser(
                    context, username, email, password, navController,
                    onError = {
                        errorMessage = it
                        coroutineScope.launch {
                            snackbarHostState.showSnackbar(it)  // Display Snackbar with error message
                        }
                    },
                    snackbarHostState
                )
    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                textStyle = TextStyle(color = Color.Black),
                colors = OutlinedTextFieldDefaults.colors(
                    focusedBorderColor = if (emailError != null) Color.Red else Color.Blue,  // Border color when focused
                ),
                modifier = Modifier
                    .fillMaxWidth()
                    .padding(start = 20.dp, top = 20.dp, end = 20.dp)
